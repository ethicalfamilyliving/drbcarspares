...
class ItemBuilderModel extends WPL_Model {
	...
	function buildItem( $id, $session, $reviseItem = false, $preview = false )
	{
		...
		$item = $this->setQuantity( $item, $post_id, $profile_details );
    ...
    public function setQuantity( $item, $post_id, $profile_details ) {
        $wc_product = wc_get_product( $post_id );
...
//        }
	...
	public function buildProductOptions( $id, $item, $post_id, $profile_details, $listing, $hasVariations, $isVariation ) {
		...
		if ( @$profile_details['condition_description'] != '' && $item->getPrimaryCategory() && !in_array( $item->getPrimaryCategory()->getCategoryID(), EbayCategoriesModel::getTradingCardsCategories() ) ) {
	...
	public function buildProfileOptions( $item, $profile_details ) {
		...
		if ( $profile_details['condition_id'] != 'none' ) {
			...
			if ( $item->getPrimaryCategory() && in_array($item->getPrimaryCategory()->getCategoryID(), EbayCategoriesModel::getTradingCardsCategories() ) ) {
				$conditionDescContainer = new ConditionDescriptorsType();
				if ( $item->ConditionID == 4000 ) {
					
					$conditionType = new ConditionDescriptorType();
					$conditionType->setName('40001');
					$conditionType->setValue($profile_details['condition_description']);
					$conditionDescContainer->addConditionDescriptor( $conditionType );
					//$item->setConditionDescriptors( $conditionDescContainer );
					$item->ConditionDescriptors = $conditionDescContainer;
				} elseif ( $item->ConditionID == 2750 ) {
					// Professional Grader
					if ( !empty( $profile_details['professional_grader'] ) ) {
						$conditionType = new ConditionDescriptorType();
						$conditionType->setName('27501');
						$conditionType->setValue($profile_details['professional_grader']);
						$conditionDescContainer->addConditionDescriptor( $conditionType );
					}

					// Grade
					if ( !empty( $profile_details['grade'] ) ) {
						$conditionType = new ConditionDescriptorType();
						$conditionType->setName('27502');
						$conditionType->setValue($profile_details['grade']);
						$conditionDescContainer->addConditionDescriptor( $conditionType );
					}

					// Certification Number
					if ( !empty( $profile_details['certification_number'] ) ) {
						$conditionType = new ConditionDescriptorType();
						$conditionType->setName('27503');
						$conditionType->setAdditionalInfo( $profile_details['certification_number'] );
						$conditionDescContainer->addConditionDescriptor( $conditionType );
					}
	...
	public function buildItemSpecifics( $id, $item, $listing, $post_id ) {
        ...
        foreach ($attributes as $name => $value) {
    		...
    		foreach ($values as $value) {
                ...
                $value = $this->processSizeMapReplacements( $name, $value, $listing['profile_data']['details'] );
	...
	public function buildCompatibilityList( $id, $item, $listing, $post_id ) {
		...
		$compatibility_list   = wple_get_compatibility_list( $post_id );
		$compatibility_names  = wple_get_compatibility_names( $post_id );
    ...
    public function buildVariations( $id, $item, $profile_details, $listing, $session ) {
        ...
        foreach ($variations as $var) {
            ...
            foreach ($var['variation_attributes'] as $name => $value) {
                ...
                $value = $this->processSizeMapReplacements( $name, $value, $profile_details);
