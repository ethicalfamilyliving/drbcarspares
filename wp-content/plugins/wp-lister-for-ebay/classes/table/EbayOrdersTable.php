...
class EbayOrdersTable extends WP_List_Table {
    ...
    function column_details($item){
        ...
        if ( isset( $_REQUEST['order_status'] ))    $page .= '&order_status='.esc_attr(wple_clean($_REQUEST['order_status']));
        if ( isset( $_REQUEST['has_wc_order'] ))    $page .= '&has_wc_order='.esc_attr(wple_clean($_REQUEST['has_wc_order']));
	    ...
	    if ( WPLE_IS_LITE_VERSION ) {
		    unset( $actions['wple_create_order'] );
		    //$title .= '<div class="upgrade-notice">Upgrade to PRO to sync this order</div>';
		    $actions['wple_upgrade'] = '<div class="upgrade-notice">'. __( 'Upgrade to PRO to sync this order', 'wp-lister-for-ebay' ) .'</div>';
	    }
